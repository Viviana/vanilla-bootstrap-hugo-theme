...
For instance, the original file path is: "C:\Users\bijou\OneDrive - University of Massachusetts\SPP Stats 608 2021\Labs\Data sets\birthweight_smoking.csv". You will change to the following and replace the backward slash '\' to '/'. 
...
```{r}
data$smoker <- as.factor(data$smoker)   # change the data structure to a factor variable in R

str(data$smoker)  # Now, check if the data structure for `smoker` has changed 

summary(data$smoker)  # See how the summary statistics has changed. 
...
```{r warning=FALSE, message=FALSE , eval=TRUE}
library(dplyr)
data <- data %>% rename_at("ï..nprevist",~"nprevist")  # rename_at("old name), ~"new name") # make sure R got the new name for nprevist
